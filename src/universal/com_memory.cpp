...
void __cdecl Hunk_ClearToMarkLow(int32_t mark)
{
    ...
    endBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[hunk_low.temp + 4095] & 0xFFFFF000);
    hunk_low.temp = mark;
    hunk_low.permanent = mark;
    Hunk_ClearData();
    beginBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[hunk_low.temp + 4095] & 0xFFFFF000);
    if (endBuf != beginBuf)
...
uint8_t* __cdecl Hunk_AllocAlign(uint32_t size, int32_t alignment, const char* name, int32_t type)
{
    ...
    endBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[s_hunkTotal - hunk_high.permanent] & 0xFFFFF000);
    ...
    if (endBuf != (uint8_t*)((uintptr_t)buf & 0xFFFFF000))
        Z_VirtualCommit((void*)((uintptr_t)buf & 0xFFFFF000), (int)(uintptr_t)&endBuf[-(int)((uintptr_t)buf & 0xFFFFF000)]); // KISAKHACK: was sus int32_t cast
...
uint32_t __cdecl Hunk_AllocateTempMemoryHigh(int32_t size, const char* name)
{
    ...
    endBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[s_hunkTotal - hunk_high.temp] & 0xFFFFF000);
    hunk_high.temp += size;
    ...
    if (endBuf != (uint8_t*)(uintptr_t)(buf & 0xFFFFF000))
        Z_VirtualCommit((void*)(uintptr_t)(buf & 0xFFFFF000), (int)(uintptr_t)&endBuf[-(int)(buf & 0xFFFFF000)]); // KISAKHACK: was sus int32_t cast
...
void Hunk_ClearTempMemoryHigh()
{
    ...
    beginBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[s_hunkTotal - hunk_high.temp] & 0xFFFFF000);
    hunk_high.temp = hunk_high.permanent;
...
uint8_t* __cdecl Hunk_AllocLowAlign(uint32_t size, int32_t alignment, const char* name, int32_t type)
{
    ...
    beginBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[hunk_low.permanent + 4095] & 0xFFFFF000);
...
void __cdecl Hunk_UserReset(HunkUser* user)
{
    ...
    pos = (void*)(uintptr_t)(((uintptr_t)&user[114].name + 3) & 0xFFFFF000);
    if (pos != (void*)(uintptr_t)((user->pos + 4095) & 0xFFFFF000))
    {
        if (user->pos - (int)(uintptr_t)pos <= 0)
            MyAssertHandler(".\\universal\\com_memory.cpp", 3019, 0, "%s", "user->pos - pos > 0");
        Z_VirtualDecommit(pos, user->pos - (uint32_t)(uintptr_t)pos);
    }
    user->pos = (int)(uintptr_t)user->buf;
